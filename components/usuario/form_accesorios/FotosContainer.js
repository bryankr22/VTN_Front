...
import ImageUpload from "../form_vehiculo/ImageUpload";

const listMobile = [...Array(5)].map((_, index) => {
  if (index === 0) return { principal: true, index };
  return { index };
});

export default function FotosContainer({ isMobile }) {
      ...
      <Grid columns={3} divided>
        ...
        {isMobile ? (
          ...
          <Grid.Row stretched>
              ...
              <Segment>
                <Dropzone
                  getUploadParams={({ file, meta }) =>
                    getUploadParamsAcc(file, meta, 0)
                  }
                  onChangeStatus={({ file, meta }, status) =>
                    handleChangeStatus(status, 0)
                  }
                  maxFiles={1}
                  accept="image/*"
                  styles={{
            ...
            <Grid.Column className="columDrop">
              <Segment style={{ paddingBottom: 0, marginBottom: 5 }}>
                <Dropzone
                  getUploadParams={({ file, meta }) =>
                    getUploadParamsAcc(file, meta, 1)
                  }
                  onChangeStatus={({ file, meta }, status) =>
                    handleChangeStatus(status, 1)
                  }
                  maxFiles={1}
                  accept="image/*"
                  styles={{
              ...
              <Segment style={{ paddingTop: 0, marginTop: 0 }}>
                <Dropzone
                  getUploadParams={({ file, meta }) =>
                    getUploadParamsAcc(file, meta, 2)
                  }
                  onChangeStatus={({ file, meta }, status) =>
                    handleChangeStatus(status, 2)
                  }
                  maxFiles={1}
                  accept="image/*"
                  styles={{
            ...
            <Grid.Column className="columDrop">
              <Segment style={{ paddingBottom: 0, marginBottom: 5 }}>
                <Dropzone
                  getUploadParams={({ file, meta }) =>
                    getUploadParamsAcc(file, meta, 3)
                  }
                  onChangeStatus={({ file, meta }, status) =>
                    handleChangeStatus(status, 3)
                  }
                  maxFiles={1}
                  accept="image/*"
                  styles={{
              ...
              <Segment style={{ paddingTop: 0, marginTop: 0 }}>
                <Dropzone
                  getUploadParams={({ file, meta }) =>
                    getUploadParamsAcc(file, meta, 4)
                  }
                  onChangeStatus={({ file, meta }, status) =>
                    handleChangeStatus(status, 4)
                  }
                  maxFiles={1}
                  accept="image/*"
                  styles={{
