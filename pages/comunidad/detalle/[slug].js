...
export default function detalle({ data }) {
    ...
    const makeComment = () => {
        ...
        let dataReq = { id: user_id, comentario, idPregunta: pregunta.id }
        axios.post(AUTH_URL + comment_api, dataReq, config).then((res) => {
            setLoading(false);
            location.reload();
        }).catch(error => {
            setLoading(false);
        ...
        <PublicLayout>
            ...
            <Container style={{ paddingTop: 25 }} text>
                ...
                {cookies.vtn_token &&
                    <Form>
                        <TextArea 
                        onChange={(e, {value}) => setComentario(value)}
                        style={{ minHeight: 100, marginBottom: 10 }} 
                        placeholder='Comentario...' />
                        <Button 
                        onClick={() => makeComment()}
                        primary floated='right' 
                        style={{ marginBottom: 10 }}>ENVIAR COMENTARIO</Button>
                    </Form>
                }
